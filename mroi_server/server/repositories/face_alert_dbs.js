...
class AlertDatabasesRepository extends BaseRepository {
  ...
  countAllRows({  }, schema=DB_SCHEMA, plain = false) {
    return models.face_alert_dbs.schema(schema)
      .count({ where: {} });     
  }// END countAllRows()
  ...
  async createAlertDB(
    ...
    try {
      const face_alert_dbs = await models.face_alert_dbs.schema(schema).create({
      ...
      return this._postFetchData(face_alert_dbs, plain);
    } catch (e) {
      throw e;
    }
  }//END createAlertDB()
...
}// END Class AlertDatabasesRepository

export default AlertDatabasesRepository;
